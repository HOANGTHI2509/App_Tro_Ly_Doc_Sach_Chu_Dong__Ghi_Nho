...
class _AdminUsersScreenState extends State<AdminUsersScreen> {
  ...
  Future<void> _updateUser(String userId, Map<String, dynamic> data) async {
    try {
      ...
      debugPrint('[AdminUsers] Update error: $e');
      if (mounted) {
        _showSnack('Lỗi cập nhật CSDL: Vui lòng kiểm tra lại cột trong Database (VD: is_verified, role, status đã có chưa?).', Colors.red.shade800);
      }
      rethrow;
    }
  }
  ...
  void _showSettingsSheet(Map<String, dynamic> user) {
    ...
    final String fullName = user['name'] ?? 'Người dùng';
  ...
  Widget _buildUserCard(Map<String, dynamic> user) {
    final String fullName = user['name'] ?? 'Chưa cập nhật';
    final String email = user['email'] ?? 'Không có email';
    final String avatar = user['avatar_url'] ?? 'https://ui-avatars.com/api/?name=$fullName&background=0C4A6E&color=fff';
